...
class TestADQLMAssistant(unittest.TestCase):
    ...
    @patch('adqlm.llm.LLMClient.generate_query')
    @patch('adqlm.datalab.NOIRLabService.execute_query')
    @patch('adqlm.llm.LLMClient.explain_result')
    def test_process_query_success(self, mock_explain, mock_execute, mock_generate_query, mock_retrieve):
        # Setup mocks
        mock_retrieve.return_value = [{'text': 'context', 'source': 'url'}]
        mock_generate_query.return_value = "SELECT * FROM stars LIMIT 10"
    ...
    @patch('adqlm.rag.DocumentEmbedder.retrieve')
    @patch('adqlm.llm.LLMClient.generate_query')
    def test_process_query_sql_generation_failure(self, mock_generate_query, mock_retrieve):
        # Setup mocks
        mock_retrieve.return_value = []
        mock_generate_query.return_value = "ERROR: Insufficient context."
